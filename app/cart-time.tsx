...
export default function CartTimeScreen() {
  return (
    <Phone bordered contentStyle={styles.phone}>
      <TopBar title="Carrinho" />
      <View style={styles.content}>
        <Text style={styles.mode}>Economia por Tempo</Text>
        <Card style={styles.table}>
          <View style={styles.headerRow}><Text style={styles.cellProduct}>Produto</Text><Text style={styles.cell}>Angeloni</Text><Text style={styles.cell}>Imperatriz</Text><Text style={styles.cell}>Carrefour</Text></View>
          {cartItems.map((r) => <View key={r.item} style={styles.row}><Text style={styles.cellProduct}>{r.item.replace(/^[^ ]+ /, '')}</Text><Text style={[styles.cell, styles.highlight]}>{r.a}</Text><Text style={styles.cell}>{r.b}</Text><Text style={styles.cell}>{r.c}</Text></View>)}
          <View style={styles.spacer} />
          <View style={styles.total}><Text style={styles.cellProduct}>TOTAL</Text><Text style={styles.cell}>R$ 98,00</Text><Text style={styles.cell}>R$ 123,00</Text><Text style={styles.cell}>R$ 133,00</Text></View>
        </Card>
        <DeliveryBox />
        <PrimaryButton label="finalizar" href="/delivery-success" style={styles.button} />
      </View>
...
const styles = StyleSheet.create({
  ...
  cell: { width: 76, color: '#333333', fontSize: 10, textAlign: 'center', height: '100%', textAlignVertical: 'center', borderLeftWidth: 1, borderColor: '#E5E5E5' },
  highlight: { backgroundColor: '#EDF9F5' },
